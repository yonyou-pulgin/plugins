...
const insertField = async (isNewRecordConfirm, isVerifyIdentity, configFields = [], signType = 0) => {
  ...
  for (const item of configFieldsPromise) {
    ...
    if (!isNewRecordConfirm && !isVerifyIdentity) {
      ...
      fieldArr.push(addField(currentTableId, `${formulaUrlEmp}&field_id=${item.signPeopleFieldId}`, successRecords, '签字确认'))
    } else if (!isNewRecordConfirm && isVerifyIdentity) {
      // 有身份 、无授权 插入链接、二维码
      fieldArr.push(addField(currentTableId, `${createUserViewUrl}&field_id=${item.signPeopleFieldId}`, successRecords, '签字确认结果', `请把链接发给签字人员：${loginUrl}`))
      fieldArr.push(addImgField(currentTableId, qrUrl, successRecords))
    } else {
      // 有授权  插入公式、状态
      if (isVerifyIdentity) {
        fieldArr.push(addSingleSelectField(currentTableId))
        fieldArr.push(addFormulaField(currentTableId, `${formulaUrl}&field_id=${item.signPeopleFieldId}`, '签字确认结果', `请把链接发给签字人员：${loginUrl}`))
        if (formulaLink) fieldArr.push(addFormulaLinkField(currentTableId, `${loginUrl}&field_id=${item.signPeopleFieldId}`, '自动化签字链接', false))
      } else {
        fieldArr.push(addSingleSelectField(currentTableId))
        fieldArr.push(addFormulaField(currentTableId, `${formulaUrlEmp}&field_id=${item.signPeopleFieldId}`, '签字确认'))
        if (formulaLink) fieldArr.push(addFormulaLinkField(currentTableId, `${formulaUrlEmp}&field_id=${item.signPeopleFieldId}`,))
      }
    }
